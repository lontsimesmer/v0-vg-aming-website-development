...
class MTNMobileMoneyService {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    const auth = Buffer.from(
      `${MTN_CONFIG.apiKey}:${MTN_CONFIG.apiSecret}`,
    ).toString("base64");
    ...
    if (!response.ok) {
      throw new Error(
        `MTN Auth failed: ${response.status} ${response.statusText}`,
      );
    }
  ...
  async requestPayment(
    ...
    try {
      ...
      throw new Error(
        `MTN Payment request failed: ${errorData.message || response.statusText}`,
      );
    } catch (error) {
