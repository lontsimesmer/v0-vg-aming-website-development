...
class OrangeMoneyService {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    if (!response.ok) {
      throw new Error(
        `Orange Auth failed: ${response.status} ${response.statusText}`,
      );
    }
  ...
  async requestPayment(
    ...
    try {
      ...
      const response = await fetch(
        `${ORANGE_CONFIG.baseUrl}/orange-money-webpay/dev/v1/webpayment`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Orange Payment request failed: ${errorData.message || response.statusText}`,
        );
      }
      ...
      if (data.status === "INITIATED") {
        return {
          ...
          message:
            "Payment initiated. User will receive SMS to complete payment.",
        };
      }
  ...
  async checkPaymentStatus(orderId: string): Promise<OrangePaymentResponse> {
    try {
      ...
      const response = await fetch(
        `${ORANGE_CONFIG.baseUrl}/orange-money-webpay/dev/v1/transactionstatus/${orderId}`,
        {
          method: "GET",
          headers: {
            Authorization: `Bearer ${token}`,
